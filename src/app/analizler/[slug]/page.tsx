...
export function generateStaticParams() {
  return caseAnalyses.map((item) => ({ slug: item.slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params
  const item = getCaseAnalysis(slug)
  if (!item) return {}

  return {
    title: `${item.title} — Analiz`,
    description: item.summary,
    alternates: { canonical: `/analizler/${item.slug}` },
  }
}
    ...
    itemListElement: [
      {
        ...
        position: 1,
        name: 'Analizler',
        item: `${siteUrl}/analizler`,
      },
      {
        '@type': 'ListItem',
        position: 2,
        name: item.shortTitle,
        item: `${siteUrl}/analizler/${item.slug}`,
      },
    ],
  }
      ...
      <article>
        <header className="hero-shell case-detail-hero">
          <div className="wrap case-detail-head">
            <a className="crumb" href="/analizler">
              <i aria-hidden="true" />
              ANALİZLER / GERİ DÖN
            </a>
            <div className="case-disclosure case-disclosure-hero" role="note">
              <strong>GERÇEK PROJE · ANONİMLEŞTİRİLMİŞ VERİ</strong>
        ...
        <section className="section-band band-paper">
          <div className="wrap section case-reading-grid">
            <aside className="case-toc" aria-label="Analiz içindekiler">
            ...
            <div className="case-body">
              ...
              <section id="hesap" className="case-copy-section">
                ...
                <p>
                  Tüm tutarlar TL'dir. Müşteri gizliliği için bazı sayısal değerler yeniden
                  ölçeklendirilmiştir. Eksi değerler maliyet veya harcamayı gösterir.
                </p>
                  ...
                  {[item.baseline, item.revised].map((scenario) => (
                    <div className="case-table-wrap" key={scenario.label}>
                      <div className="case-table-head">
                        <div>
                          <span>KARŞILAŞTIRMA DÖNEMİ</span>
              ...
              <section id="sinirlar" className="case-copy-section">
                ...
                <div className="case-limit-panel">
                  <strong>Bu analiz tek başına nedensel etki kanıtı değildir.</strong>
                  <ul>
