    ...
    <main className="page">
      ...
      <section className="wrap hero single comparison-index-hero">
        <div>
          ...
          <p className="intro">
            Formülü ezberlemek yerine kapsamı, veri kaynağını, güçlü olduğu kararı ve
            yanıltabileceği noktayı karşılaştırın.
          </p>
        </div>
      </section>
      ...
      <section className="wrap comparison-index">
        <div className="comparison-index-note">
          ...
          <span>
            Sponsorlu içerik veya affiliate bağlantı içermez. Kaynak ve son doğrulama tarihi her
            sayfada görünür.
          </span>
        </div>
          ...
          {metricComparisons.map((item, index) => (
            <a key={item.slug} href={`/karsilastirmalar/${item.slug}`} className="comparison-card">
              ...
              <dl>
                <div>
                  <dt>Sol metrik</dt>
                  <dd>{item.left.name}</dd>
                </div>
                <div>
                  <dt>Sağ metrik</dt>
                  <dd>{item.right.name}</dd>
                </div>
              </dl>
