...
function scenarioRows(scenario: CaseScenario) {
  ...
  return [
    ...
    ...scenario.costs.map((cost) => ({ label: cost.label, value: -cost.amount, formula: cost.note })),
    { label: 'Reklam öncesi katkı', value: metrics.preAdContribution, formula: 'Net satış − değişken giderler' },
    { label: 'Reklam harcaması', value: -scenario.adSpend, formula: `Net satış ÷ ${metrics.roas.toFixed(1)} ROAS` },
    { label: 'Reklam sonrası katkı', value: metrics.postAdContribution, formula: `Net satışın ${percent(metrics.postAdContributionMargin)}` },
  ]
}
...
export default async function DemoCasePage({ params }: PageProps) {
  ...
  const breadcrumbLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Vaka Analizleri', item: `${siteUrl}/vaka-analizleri` },
      { '@type': 'ListItem', position: 2, name: item.shortTitle, item: `${siteUrl}/vaka-analizleri/${item.slug}` },
    ],
  }
      ...
      <article>
        <header className="hero-shell case-detail-hero">
          <div className="wrap case-detail-head">
            <a className="crumb" href="/vaka-analizleri"><i aria-hidden="true" />VAKA ANALİZLERİ / GERİ DÖN</a>
            ...
            <div className="case-hero-meta">
              <span><small>KARŞILAŞTIRMA</small>{item.duration}</span>
              <span><small>ROAS</small>{baseline.roas.toFixed(1)} → {revised.roas.toFixed(1)}</span>
              <span><small>ÖRNEK SONUÇ</small>{percent(contributionChange)} katkı değişimi</span>
              <span><small>GÜNCELLEME</small>{new Intl.DateTimeFormat('tr-TR').format(new Date(item.updatedAt))}</span>
            </div>
          </div>
        </header>
          ...
          <div className="wrap section case-reading-grid">
            ...
            <div className="case-body">
              <section id="senaryo" className="case-copy-section">
                ...
                <p>Bu laboratuvarın sorusu budur. Baz senaryoda net satış 1 milyon TL ve reklam panelindeki ROAS 4,2’dir. Revize senaryoda net satış daha düşük olmasına rağmen sipariş karmasının değiştiği varsayılır. İki dönemde ROAS sabit tutularak kâr farkının medya veriminden değil, birim ekonomiden gelebileceği görünür hâle getirilir.</p>
                <div className="case-answer">
                  <span>60 SANİYELİK CEVAP</span>
                  <strong>Aynı ROAS, aynı kâr demek değildir.</strong>
                  <p>Ürün maliyeti, komisyon, lojistik ve iade yükü değiştiğinde her reklam lirasından sonra kalan katkı da değişir.</p>
                </div>
              </section>
              ...
              <section id="veri" className="case-copy-section">
                ...
                <div className="case-source-grid">
                  {item.dataSources.map((source, index) => <div key={source}><span>0{index + 1}</span><p>{source}</p></div>)}
                </div>
              ...
              <section id="teshis" className="case-copy-section">
                ...
                <div className="case-diagnosis">
                  {item.diagnosis.map((step, index) => (
                    <div key={step.title}><span>0{index + 1}</span><div><h3>{step.title}</h3><p>{step.detail}</p></div></div>
                  ))}
                </div>
              </section>
              ...
              <section id="hesap" className="case-copy-section">
                ...
                <p>Tüm tutarlar TL ve tüm sonuçlar simülasyondur. Eksi değerler maliyet veya harcamayı gösterir.</p>
                <div className="case-tables">
                  {[item.baseline, item.revised].map((scenario) => (
                    <div className="case-table-wrap" key={scenario.label}>
                      <div className="case-table-head"><div><span>SİMÜLASYON</span><h3>{scenario.label}</h3></div><small>{scenario.period}</small></div>
                      <table>
                        <thead><tr><th scope="col">Kalem</th><th scope="col">Formül / kapsam</th><th scope="col">Tutar</th></tr></thead>
                        <tbody>
                          {scenarioRows(scenario).map((row) => (
                            <tr key={row.label} className={row.label === 'Reklam sonrası katkı' ? 'case-total-row' : undefined}>
                              <th scope="row">{row.label}</th><td>{row.formula}</td><td>{money(row.value)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                <div className="case-result-strip">
                  <div><span>BAZ KATKI</span><strong>{money(baseline.postAdContribution)}</strong><small>{percent(baseline.postAdContributionMargin)} marj</small></div>
                  <i aria-hidden="true">→</i>
                  <div><span>REVİZE KATKI</span><strong>{money(revised.postAdContribution)}</strong><small>{percent(revised.postAdContributionMargin)} marj</small></div>
                  <div className="case-result-change"><span>SİMÜLASYON SONUCU</span><strong>{percent(contributionChange)}</strong><small>reklam sonrası katkı değişimi</small></div>
                </div>
                <div className="case-formula">
                  <span>TEKRARLANABİLİR FORMÜL</span>
                  <code>Reklam sonrası katkı = Net satış − Ürün maliyeti − Komisyon/ödeme − Kargo/paketleme − İade rezervi − Reklam harcaması</code>
                </div>
              </section>
              ...
              <section id="kararlar" className="case-copy-section">
                ...
                <div className="case-decision-grid">
                  {item.decisions.map((decision, index) => <div key={decision.title}><span>0{index + 1}</span><h3>{decision.title}</h3><p>{decision.detail}</p></div>)}
                </div>
              </section>
        ...
        <section className="section-band band-cyan">
          <div className="wrap section">
            <div className="head"><div><span className="eyebrow">KENDİ VERİNİZLE</span><h2>Modeli araçlarda yeniden kurun.</h2></div><p>Varsayımları kendi ürün maliyeti ve reklam verinizle değiştirin.</p></div>
            <div className="grid case-related-grid">
              {item.relatedTools.map((link) => <a className="card" key={link.href} href={link.href}><span className="tag">ÜCRETSİZ ARAÇ</span><h3>{link.title}</h3><span className="link">Aracı aç →</span></a>)}
