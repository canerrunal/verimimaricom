...
function clampBar(deltaPercent: number) {
  return Math.min(100, Math.max(5, Math.abs(deltaPercent)))
}

export default function MetricsBoard({ metrics = [] }: { metrics?: any[] }) {
  return (
    <section aria-label="Metrik görselleştirme">
      <div className="metrics-grid" style={{ gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
