  ...
  return (
    <section className="wrap hero">
      <div>
        ...
        <h1>
          E-ticaret verisini <span className="accent">ölçülebilir</span> kâr kararlarına dönüştüren araçlar.
        </h1>
        <p className="intro">
          Reklam maliyetiniz, ürün marjınız ve operasyon giderleriniz üzerinden hangi kararın mantıklı olduğunu görün. Tahmine değil, hesaplamaya dayanın.
        </p>
  ...
  return (
    <div className="console-body">
      ...
      <div className="row">
        ...
        <input type="number" value={sale} onChange={(e) => setSale(Math.max(0, Number(e.target.value) || 0))} />
      </div>
      <div className="row">
      ...
      <div className="row">
        ...
        <input type="number" value={comm} onChange={(e) => setComm(Math.max(0, Number(e.target.value) || 0))} />
      </div>
      <div className="row">
        <span>Kargo + paketleme</span>
        <input type="number" value={ship} onChange={(e) => setShip(Math.max(0, Number(e.target.value) || 0))} />
      </div>
      <div className="row">
        <span>Beklenen iade maliyeti</span>
        <input type="number" value={ret} onChange={(e) => setRet(Math.max(0, Number(e.target.value) || 0))} />
      </div>
